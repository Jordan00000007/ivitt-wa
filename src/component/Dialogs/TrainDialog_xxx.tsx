...
import { createTrainingAPI, downloadPreTrainedAPI, getBatchSizeAPI, getDefaultParamAPI, getModelAPI, getTrainMethodListAPI, startTrainingAPI, addTrainingTaskAPI } from '../../constant/API/trainAPI';
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        addTrainingTaskAPI(id, postData)
            .then(({ data }) => {
                ...
                getDatasetListAPI(id)
                .then(({ data }) => {

                    console.log('getIterListCallback---------------')
                    console.log(data.data)

                    const theLastIter = 'iteration'+data.data.folder_name.length;
                    dispatch(setIteration(theLastIter));
                    dispatch(setIsTraining('start'));
                ...
                .catch(({ response }) => console.log('getDatasetListAPI-Error', response.data))
                ...
                // createTrainingAPI(id, postData.training_parameter)
                //     .then(({ data }) => {
                //         handleCancel();
                //         //透過改變redux的Iteration & CurrentTab去導到Model的training iter

                //         console.log('my last iter is ', data.data.iter_name)
                ...
                //                 console.log('getDatasetListAPI-Success', data);
                //                 //log('getLastIter-Success', data.data.folder_name);
                //                 const IterArr = filter(data.data.folder_name, function (o) { return o !== 'workspace' });
                //                 const myLastIter = `iteration${IterArr.length + 1}`;

                //                 console.log('myLastIter   ---> ', myLastIter);

                //                 //dispatch(setIteration(myLastIter));
                //                 //dispatch(setCurrentTab("Model"));
                //                 // if (IterArr.length > 0) {
                //                 //     const myLastIter = IterArr[IterArr.length - 1];
                //                 //     console.log('last Iter = ',myLastIter)
                //                 //     dispatch(setIteration(myLastIter));
                //                 //     dispatch(setCurrentTab("Model"));
                ...
                //             })
                //             .catch((err) => {
                //                 console.log('getDatasetListAPI-Err', err);
                //             })
